...
for basin in "${basins[@]}"; do
    for n_cluster in "${n_clusters[@]}"; do
        ...
        python3 build_dataset.py --n_clusters $n_cluster --basin $basin --anomaly_clustering 'n'
        echo "Building dataset for $basin with $n_cluster anomaly clusters"
        python3 build_dataset.py --n_clusters $n_cluster --basin $basin --anomaly_clustering 'y'
    done
done