...
def main(n_clusters, n_vars, n_idxs, results_folder, basin, model_kind, n_folds, start_year, end_year):
    ...
    plt.plot(Y_pred_MLP_noFS_seasonal.index, Y_pred_MLP_noFS_seasonal['tcg'], label=f'NoFS-MLP - R:{r_mlp_noFS:.3f}', color='#9467bd', linestyle='--', linewidth=2)
    plt.plot(Y_pred_XGB_seasonal.index, Y_pred_XGB_seasonal['tcg'], label=f'FS-XGB - R:{r_xgb:.3f}', color='#2ca02c', linewidth=2)
    plt.plot(Y_pred_XGB_noFS_seasonal.index, Y_pred_XGB_noFS_seasonal['tcg'], label=f'NoFS-XGB - R:{r_xgb_noFS:.3f}', color='#8c564b', linestyle='--', linewidth=2)
    plt.plot(Y_pred_LGBM_seasonal.index, Y_pred_LGBM_seasonal['tcg'], label=f'FS-LGBM - R:{r_lgbm:.3f}', color='#d62728', linewidth=2)
    plt.plot(Y_pred_LGBM_noFS_seasonal.index, Y_pred_LGBM_noFS_seasonal['tcg'], label=f'NoFS-LGBM - R:{r_lgbm_noFS:.3f}', color='#e377c2', linestyle='--', linewidth=2)
    plt.grid(True, which='both', linestyle='--', linewidth=0.5)
    plt.xlabel('Months')
    plt.ylabel('# of TCs per month')
    plt.legend()
    ...
    plt.plot(Y_pred_MLP_noFS_annual.index.year, Y_pred_MLP_noFS_annual['tcg'], label=f'NoFS-MLP - R:{r_mlp_noFS:.3f}', color='#9467bd', linestyle='--', linewidth=2)
    plt.plot(Y_pred_XGB_annual.index.year, Y_pred_XGB_annual['tcg'], label=f'FS-XGB - R:{r_xgb:.3f}', color='#2ca02c', linewidth=2)
    plt.plot(Y_pred_XGB_noFS_annual.index.year, Y_pred_XGB_noFS_annual['tcg'], label=f'NoFS-XGB - R:{r_xgb_noFS:.3f}', color='#8c564b', linestyle='--', linewidth=2)
    plt.plot(Y_pred_LGBM_annual.index.year, Y_pred_LGBM_annual['tcg'], label=f'FS-LGBM - R:{r_lgbm:.3f}', color='#d62728', linewidth=2)
    plt.plot(Y_pred_LGBM_noFS_annual.index.year, Y_pred_LGBM_noFS_annual['tcg'], label=f'NoFS-LGBM - R:{r_lgbm_noFS:.3f}', color='#e377c2', linestyle='--', linewidth=2)
    plt.grid(True, which='both', linestyle='--', linewidth=0.5)
    plt.xlabel('Years')
