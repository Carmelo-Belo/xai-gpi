...
def main(predictors_type, n_idxs):
    ...
    for basin, (basin_name, fs_model, n_clusters) in basin_dict.items():
        ...
        if predictors_type == 'deseason':
            ...
            str_pt = 'nc'
        track_df = track_df[track_df.index.str.contains(f'nd{n_idxs}')] # get only simulations with that number of climate indices
        ...
        csv_path = os.path.join(results_dir, f'selected_features_best_models_{basin}_{str_pt}{n_clusters}_{n_idxs}idxs.csv')
        ...
        for c, column in enumerate(df_perc_sel.columns):
            ...
            perc_fig_dir = os.path.join(basin_dir, f'sel_var_{str_pt}{n_clusters}_{n_idxs}idxs_perc{column}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--predictors_type', default='original', type=str, help='Type of predictors: original, deseason, or detrend.')
    parser.add_argument('--n_idxs', type=int, help='Number of climate indices and teleconnections considered as candidate features.')
    args = parser.parse_args()
    main(args.predictors_type, args.n_idxs)