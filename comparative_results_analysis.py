...
def main(basin, n_vars, n_idxs):
    ...
    for n_cluster in n_clusters:
        ...
        heatmap, df60 = ut.vars_selection_heatmaps(experiments_folders_NC, n_cluster, selected_vars_df_list_NC, atm_vars, idx_vars, display_percentage=True)
        heatmap.savefig(os.path.join(sub_comp_results_dir, f'all_heatmaps_NC_{n_vars}vars_{n_idxs}idxs.pdf'), format='pdf', dpi=300)
        df60.to_csv(os.path.join(sub_comp_results_dir, f'all_60%selvar_NC_{n_vars}vars_{n_idxs}idxs.csv'))
        plt.close(heatmap)
        heatmap, df60 = ut.vars_selection_heatmaps(experiments_folders_linreg_NC, n_cluster, selected_vars_df_list_linreg_NC, atm_vars, idx_vars, display_percentage=True)
        heatmap.savefig(os.path.join(sub_comp_results_dir, f'linreg_heatmaps_NC_{n_vars}vars_{n_idxs}idxs.pdf'), format='pdf', dpi=300)
        df60.to_csv(os.path.join(sub_comp_results_dir, f'linreg_60%selvar_NC_{n_vars}vars_{n_idxs}idxs.csv'))
        plt.close(heatmap)
        heatmap, df60 = ut.vars_selection_heatmaps(experiments_folders_lgbm_NC, n_cluster, selected_vars_df_list_lgbm_NC, atm_vars, idx_vars, display_percentage=True)
        heatmap.savefig(os.path.join(sub_comp_results_dir, f'lgbm_heatmaps_NC_{n_vars}vars_{n_idxs}idxs.pdf'), format='pdf', dpi=300)
        df60.to_csv(os.path.join(sub_comp_results_dir, f'lgbm_60%selvar_NC_{n_vars}vars_{n_idxs}idxs.csv'))
        plt.close(heatmap)
        # Plot heatmaps for variable selection considering all experiments, and then all experiments for linear regression and LightGBM when adopting anomaly clustering
        heatmap, df60 = ut.vars_selection_heatmaps(experiments_folders_AC, n_cluster, selected_vars_df_list_AC, atm_vars, idx_vars, display_percentage=True)
        heatmap.savefig(os.path.join(sub_comp_results_dir, f'all_heatmaps_AC_{n_vars}vars_{n_idxs}idxs.pdf'), format='pdf', dpi=300)
        df60.to_csv(os.path.join(sub_comp_results_dir, f'all_60%selvar_AC_{n_vars}vars_{n_idxs}idxs.csv'))
        plt.close(heatmap)
        heatmap, df60 = ut.vars_selection_heatmaps(experiments_folders_linreg_AC, n_cluster, selected_vars_df_list_linreg_AC, atm_vars, idx_vars, display_percentage=True)
        heatmap.savefig(os.path.join(sub_comp_results_dir, f'linreg_heatmaps_AC_{n_vars}vars_{n_idxs}idxs.pdf'), format='pdf', dpi=300)
        df60.to_csv(os.path.join(sub_comp_results_dir, f'linreg_60%selvar_AC_{n_vars}vars_{n_idxs}idxs.csv'))
        plt.close(heatmap)
        heatmap, df60 = ut.vars_selection_heatmaps(experiments_folders_lgbm_AC, n_cluster, selected_vars_df_list_lgbm_AC, atm_vars, idx_vars, display_percentage=True)
        heatmap.savefig(os.path.join(sub_comp_results_dir, f'lgbm_heatmaps_AC_{n_vars}vars_{n_idxs}idxs.pdf'), format='pdf', dpi=300)
        df60.to_csv(os.path.join(sub_comp_results_dir, f'lgbm_60%selvar_AC_{n_vars}vars_{n_idxs}idxs.csv'))
        plt.close(heatmap)
