...
def main(basin, n_clusters, res, first_year, last_year, remove_seasonality, remove_trend):
    ...
    if remove_seasonality == 'y':
        cluster_path = os.path.join(project_dir, 'xai-gpi', 'data', f'{basin}_{n_clusters}clusters_deseason')
        deseasonalize = True
        detrend = False
    elif remove_trend == 'y':
        cluster_path = os.path.join(project_dir, 'xai-gpi', 'data', f'{basin}_{n_clusters}clusters_detrend')
        deseasonalize = False
        detrend = True
    else:
